...
def build_history_page() -> None:
    ...
    with ui.column().classes("dw-main-content dw-shell gap-5"):
        ...
        def open_report(report_id: int) -> None:
            ...
            with report_dialog, ui.card().classes("dw-panel shadow-none w-[min(860px,94vw)] h-[min(88vh,860px)] p-0 overflow-hidden"):
                with ui.column().classes("h-full gap-0"):
                    with ui.row().classes("w-full items-start justify-between gap-4 p-6 pb-4 shrink-0 border-b border-[color:var(--dw-line)]"):
                        with ui.column().classes("gap-3 min-w-0 flex-1"):
                            ...
                            ui.label(report["top_risk"]).classes("text-xl font-medium dw-text leading-7")
                        ui.button(icon="close", on_click=report_dialog.close).props("flat round dense").classes("shrink-0")
                    with ui.column().classes("flex-1 min-h-0 overflow-y-auto gap-4 p-6"):
                        with ui.column().classes("gap-1"):
                            ui.label("Description").classes("text-sm font-semibold dw-text")
                            ui.label(report["top_risk"]).classes("text-sm leading-6 dw-text")
                        with ui.column().classes("gap-1"):
                        ...
                        with ui.column().classes("gap-1"):
                            ui.label("Processing summary").classes("text-sm font-semibold dw-text")
                            ui.label(report["parse_summary"]).classes("text-sm leading-6 dw-muted")
                        with ui.column().classes("gap-1"):
                            ui.label("Audit metadata").classes("text-sm font-semibold dw-text")
                            ui.label(
                            ...
                            if report.get("narrative_model") or report.get("narrative_local_mode") is not None:
                            ...
                            if llm_notice:
                                ui.label("LLM note: " + llm_notice).classes("text-sm dw-warning-text")
                            ...
                            if audit.get("files_analyzed"):
                                ui.label("Files analyzed:").classes("text-sm font-semibold dw-text mt-1")
                                        ...
                                        with ui.row().classes("w-full items-start justify-between gap-3 p-3 flex-wrap"):
                                            with ui.column().classes("min-w-0 flex-1 gap-1"):
                                                ...
                                                for security_flag in contributor.get("security_flags", []):
                                                    ui.label(security_flag).classes("text-xs dw-danger-text")
                                            render_risk_badge(contributor["severity"])
                    with ui.row().classes("w-full justify-end gap-3 p-4 shrink-0 border-t border-[color:var(--dw-line)]"):
        ...
        def render_actions() -> None:
            ...
            with actions_row:
                ...
                with ui.row().classes("items-center gap-3 flex-wrap"):
                    ...
                    ui.label(f"Showing {start}-{end} of {total_report_count} reports · {len(selected_ids)} selected").classes(
                        "text-sm dw-muted"
                    )
                with ui.row().classes("items-center justify-end gap-4 flex-wrap"):
                    ...
                    if page_state["page"] >= max(1, (total_report_count - 1) // page_state["page_size"] + 1):
                        next_button.disable()
