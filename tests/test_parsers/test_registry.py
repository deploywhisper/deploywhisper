...
""",
        ...
        self.assertEqual(batch.failed_count, 0)
        self.assertEqual(
            batch.files[0].changes[0].resource_id, "aws_security_group.cluster"
        )
        ...
        self.assertIn(
            "multiple downstream resources", batch.files[0].changes[1].summary
        )

    def test_parse_uploaded_files_formats_terraform_plan_with_resource_specific_summary(
        self,
    ) -> None:
        batch = parse_uploaded_files(
            [
        ...
        self.assertEqual(batch.parsed_count, 1)
        self.assertIn(
            "cluster or node-group behavior", batch.files[0].changes[0].summary
        )
