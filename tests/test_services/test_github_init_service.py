...
EXAMPLE_REPO_PATH = "/example-repo"
...
class GitHubInitServiceTests(unittest.TestCase):
    ...
    def test_collect_github_init_options_defaults_to_action_first_answers(self) -> None:
        ...
        self.assertEqual(options.repo_path, EXAMPLE_REPO_PATH)
        ...
        self.assertIsNone(options.project_key)
        self.assertIsNone(options.workspace_key)
        self.assertEqual(options.base_branch, "main")
        self.assertFalse(options.enable_github_app)

    def test_collect_github_init_options_accepts_optional_action_scope(self) -> None:
        responses = iter(
            [
                EXAMPLE_REPO_PATH,
                "",
    ...
    def test_run_github_init_writes_files_and_opens_pr(
        ...
        self.assertIn("Configured project key: `payments`", readme_text)
        self.assertIn("Configured workspace key: `prod`", readme_text)
        self.assertIn("APP_BASE_URL", readme_text)
    ...
    @patch("integrations.github.init_service._require_binary")
    @patch("integrations.github.init_service._run_command")
    def test_run_github_init_omits_scope_inputs_for_scope_derived_endpoint(
        self,
        run_command,
        require_binary,
    ) -> None:
        ...
        self.assertNotIn("project-key:", workflow_text)
        self.assertNotIn("workspace-key:", workflow_text)
        self.assertIn("endpoint derives project scope", readme_text)
