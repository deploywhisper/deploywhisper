...
"""
    ...
    def test_submission_manifest_preserves_ai_assistance_provenance_without_certainty(
        self,
    ) -> None:
        files = [
            (
                "main.tf",
                b"# Generated by ChatGPT; reviewer verification required\n"
        ...
        self.assertEqual(manifest.provenance["authorship"], "ai-assisted")
        self.assertEqual(
            manifest.provenance["authorship_certainty"],
            "suggested",
        )
        self.assertIn(
            "does not establish authorship",
            manifest.provenance["authorship_note"],
        )
        self.assertEqual(
            manifest.items[0].provenance["authorship"],
            "ai-assisted",
        )
