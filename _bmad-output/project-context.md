...
- For review-flow or accessibility-sensitive UI changes, also run browser validation against the compose-built app: `docker compose up -d --build`, then `BASE_URL=http://localhost:8080 npm run test:ui-review`.
- For any UI-facing story, include an explicit story task for browser validation. If the story changes a React route, UI primitive, rendered report/history/dashboard/settings/skills surface, browser interaction, keyboard behavior, or accessibility semantics, the dev agent must run Playwright validation in a browser and record the command/result in the Dev Agent Record. Use `BASE_URL=http://localhost:8080 npm run test:ui-review` for the SPA e2e lane and `BASE_URL=http://localhost:8080 RUN_UI_A11Y=1 bash scripts/ci-local.sh` for the full local UI lane after `docker compose up -d --build`. All UI browser checks use root SPA routes under `http://localhost:8080/`; do not use `npm run ui:dev` or legacy prefixed SPA routes as verification. If no UI surface is touched, record `UI validation not applicable`; do not silently skip UI validation.
...
- For changes pushed to `deploywhisper/analyze-action`, validate the pushed action from `deploywhisper/action-smoke-consumer` before declaring the story ready. For unreleased action branches, use a temporary smoke-consumer branch that points the workflow at the action feature branch, then record the GitHub Actions run URL/result in the BMad story.
