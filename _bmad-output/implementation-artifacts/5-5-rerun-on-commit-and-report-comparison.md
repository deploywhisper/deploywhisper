...
- [x] [Review][Patch] Previous reports with more than 64 unique findings still degrade to “exact counts unavailable,” so PR output does not highlight new, resolved, and persistent findings for those reruns [/private/tmp/deploywhisper-analyze-action/action_runtime.py:958]
- [x] [Review][Patch] Current scan metadata drops all finding labels once a scan has 33-64 findings, so later reruns can compute counts without naming the changed finding [/private/tmp/deploywhisper-analyze-action/action_runtime.py:642]
- [x] [Review][Patch] No-ID fallback identity uses ordered `evidence_refs`, so the same finding can become resolved/new when evidence references arrive in a different order [/private/tmp/deploywhisper-analyze-action/action_runtime.py:848]
- [x] [Review][Patch] No-ID fallback identity normalizes punctuation out of discriminator fields, so distinct paths like `env/prod.tf` and `env-prod.tf` can collapse into one comparison key [/private/tmp/deploywhisper-analyze-action/action_runtime.py:824]
...
- Latest review-fix focused validation: `python3 -m unittest tests.test_action_runtime.BuildPrCommentTests.test_build_pr_comment_compares_previous_marker_above_sixty_four_findings tests.test_action_runtime.BuildPrCommentTests.test_build_pr_comment_keeps_labels_for_previous_marker_above_thirty_two tests.test_action_runtime.BuildPrCommentTests.test_build_pr_comment_canonicalizes_evidence_refs_for_no_id_fallback tests.test_action_runtime.BuildPrCommentTests.test_build_pr_comment_preserves_path_punctuation_in_no_id_fallback -q` passed with 4 tests.
...
- Full action latest review-fix validation: `python3 -m unittest discover -s tests -q` passed with 57 tests.
...
- UI validation not applicable: no retired Python UI route, rendered app UI, browser interaction, keyboard behavior, or accessibility semantics changed in `deploywhisper/deploywhisper`.
