...
- [x] [Review][Patch] [HIGH] Model the pinned Analyze Action capability explicitly and fail on an unclassified SHA; interpolating any future pin into unconditional advisory-only generated guidance could activate inherited blocking under a false promise. [integrations/github/init_service.py:327]
- [x] [Review][Patch] [MEDIUM] Parse the documented workflow YAML examples and assert their actual `uses:` entries match both scaffold constants and that `id: deploywhisper` belongs to the Analyze Action step; whole-document substring checks can pass on provenance prose or an unrelated step while examples drift. [tests/test_docs/test_enforcement_guardrails.py:376]
- [x] [Review][Patch] [HIGH] Bound break-glass with an immutable issue/expiry pair and organization-approved maximum TTL, require new approval for extensions, name the independent delivery-freeze control, and assert the complete checklist item as one structured requirement. [docs/enforcement-guardrails.md:500]
- [x] [Review][Patch] [LOW] Preserve the four required Epic 11 story keys while also validating every dynamically discovered Epic 11 story from parsed YAML, so missing, quoted, commented, or future entries cannot evade lifecycle checks. [tests/test_docs/test_enforcement_guardrails.py:447]
- [x] [Review][Patch] [LOW] Make GitHub anchor suffix allocation collision-safe and reject invalid backtick-fence openings; headings such as `Guardrails`, `Guardrails-1`, `Guardrails` must produce distinct slugs. [tests/test_docs/test_enforcement_guardrails.py:503]
- [x] [Review][Patch] [MEDIUM] Require checkout candidate refs to be peeled to the executed commit when annotated, not merely resolved to a tag object, before diff review and pin replacement. [docs/github-action.md:110]
- [x] [Review][Patch] [MEDIUM] Enforce the Epic 11 lifecycle invariant in both directions: when every tracked Story 11.x entry is done, `epic-11` must also be done; the current test silently skips that stale-epic case. [tests/test_docs/test_enforcement_guardrails.py:440]
- [x] [Review][Patch] [HIGH] Define an out-of-band alert and delivery-freeze path when check-publication retries are exhausted; the failed check-publication channel cannot publish its own terminal operational failure. [docs/enforcement-guardrails.md:243]
- [x] [Review][Patch] [LOW] Make the section extractor Markdown-fence-aware so heading-shaped text inside examples cannot create false duplicate sections or truncate the validated section. [tests/test_docs/test_enforcement_guardrails.py:535]
- [x] [Review][Patch] [MEDIUM] Separate the encrypted immutable raw authenticated response from the redacted reviewer view and state which byte sequence the SHA-256 digest covers; redacting the only retained bytes would break exact-response reconstruction. [docs/enforcement-guardrails.md:279]
- [x] [Review][Patch] [LOW] Use the full audited Action tag-object ID in the README provenance note instead of the ambiguous seven-character abbreviation. [README.md:795]
- [x] [Review][Patch] [HIGH] Make the rollout checklist treat missing idempotency coordination or deletion/rename coverage as blockers to enabling `soft-block` or `hard-block`; an "explicit non-blocking disposition" cannot satisfy a blocking-readiness checklist item. [docs/enforcement-guardrails.md:471]
- [x] [Review][Patch] [MEDIUM] Dismiss and reacquire human approval when a report, settings snapshot, manifest, material context input, consumer revision, or workflow changes without changing the protected commit; commit-only stale-approval rules can preserve approval for a materially different decision. [docs/enforcement-guardrails.md:422]
- [x] [Review][Patch] [LOW] Require exactly one effective-status table in the mode section so a second contradictory table cannot be ignored by the parser while the documentation regression remains green. [tests/test_docs/test_enforcement_guardrails.py:546]
- [x] [Review][Patch] [HIGH] Decouple the reviewed capability registry key from `ANALYZE_ACTION_PINNED_SHA`; changing the pin in source currently changes the registry key in the same edit and silently classifies an unreviewed revision as advisory-only instead of failing closed. [integrations/github/init_service.py:31]
- [x] [Review][Patch] [HIGH] Make every generated artifact capability-aware and exercise both capability branches; an enforcement-capable pin still produces a PR body promising "advisory-only check behavior," and no regression executes the enforcement-capable README/PR path. [integrations/github/init_service.py:348]
- [x] [Review][Patch] [HIGH] Remove the CI guide's explicit-opt-in promise and require operators to inspect the resolved setting source before onboarding, because an integration without an override can already inherit a blocking project default. [docs/ci-advisory-consumption.md:83]
- [x] [Review][Patch] [HIGH] Require an independently running watchdog or required context for cancellation and timeout; `if: always()` inside the affected workflow cannot reliably publish its own terminal failure after cancellation. [docs/enforcement-guardrails.md:78]
- [x] [Review][Patch] [MEDIUM] Define the operational control that invalidates and reacquires approval after non-commit decision inputs change, such as an external approval record or independently enforced approval check keyed to the complete decision identity. [docs/enforcement-guardrails.md:463]
- [x] [Review][Patch] [HIGH] Require consumers to reject missing or malformed `should-block` outputs before exact string comparison or `fromJSON`; any value outside `"true"` and `"false"` must fail as an operational error rather than fall through as non-blocking. [docs/github-action.md:145]
- [x] [Review][Patch] [HIGH] Define reconciliation for an analysis-submission timeout after the server may have committed a report; do not retry until the external coordinator resolves the unknown outcome, and keep the consumer non-blocking where reconciliation is impossible. [docs/enforcement-guardrails.md:243]
- [x] [Review][Patch] [MEDIUM] Serialize overlapping exceptions or issue per-exception bypass capabilities so revoking one repository-wide bypass cannot terminate a valid exception or extend an expired one. [docs/enforcement-guardrails.md:273]
- [x] [Review][Patch] [MEDIUM] Make documentation regressions operate on rendered Markdown: ignore fenced code and HTML comments for prose, links, anchors, and workflow examples; recognize indented ATX and Setext boundaries; parse valid link destinations; and require resolved local targets to remain regular files inside the repository. [tests/test_docs/test_enforcement_guardrails.py:579]
- [x] [Review][Patch] [LOW] Guard the effective-status table separator lookup so a header at end-of-section raises the intended focused `AssertionError` instead of an uncaught `IndexError`. [tests/test_docs/test_enforcement_guardrails.py:713]
- [x] [Review][Patch] [HIGH] Validate capability-registry values as `AnalyzeActionCapability` members and reject every unknown value; the current identity-based fallback treats a typo or raw string as enforcement-capable guidance instead of failing closed. [integrations/github/init_service.py:50]
- [x] [Review][Patch] [HIGH] Keep generated onboarding advisory under inherited blocking when the selected Action becomes enforcement-capable; the scaffold always emits the live Action step and only asks users to keep its check non-required, so it can begin failing before an explicit narrow advisory override or completed guardrail review exists. [integrations/github/init_service.py:307]
- [x] [Review][Patch] [HIGH] Add the always-triggered terminal context and independent cancellation/timeout watchdog to the rollout checklist; they are mandatory in the normative workflow section but absent from the readiness gate. [docs/enforcement-guardrails.md:517]
- [x] [Review][Patch] [MEDIUM] Make rollout-checklist approval use the complete decision identity, not only the commit SHA, so report, manifest, settings, material-context, consumer, and workflow changes all force a new approval. [docs/enforcement-guardrails.md:559]
- [x] [Review][Patch] [MEDIUM] Define the exact material-context fields, canonical serialization/version, ordering, and digest algorithm used by the external approval identity; `material-context digest` is currently required but not reproducibly computable. [docs/enforcement-guardrails.md:490]
- [x] [Review][Patch] [MEDIUM] Require audit evidence retention through the longest exception, compliance-audit, and incident-attribution horizon; merely recording an unconstrained retention period can permit evidence to expire before review. [docs/enforcement-guardrails.md:319]
- [x] [Review][Patch] [MEDIUM] Finish the rendered-Markdown boundary by excluding inline and indented code, treating an unclosed HTML comment as hidden through EOF, and rejecting malformed destination/title suffixes; non-operative or broken content can still satisfy prose and link contracts. [tests/test_docs/test_enforcement_guardrails.py:891]
- [x] [Review][Patch] [MEDIUM] Parse rendered ATX headings and tables structurally when enforcing uniqueness; equivalent headings with closing hashes and competing mode tables with altered header labels can evade duplicate/contradiction checks. [tests/test_docs/test_enforcement_guardrails.py:753]
- [x] [Review][Patch] [MEDIUM] Specify an algorithm-qualified cryptographic digest over exact artifact bytes for non-PR target binding; the current generic `artifact digest` requirement is neither reproducible nor guaranteed collision-resistant. [docs/enforcement-guardrails.md:168]
- [x] [Review][Patch] [LOW] Resolve every repository-local link extracted from the guardrail guide, not only the four required targets, so later broken local links cannot remain green under a test named as comprehensive outbound-link validation. [tests/test_docs/test_enforcement_guardrails.py:250]
- [x] [Review][Patch] [HIGH] Synchronize the sprint-status header and parsed `last_updated` values; the exact claimed documentation/infra suite currently fails on their 2026-09-18 versus 2026-09-21 mismatch, invalidating the story's green-validation claim. [_bmad-output/implementation-artifacts/sprint-status.yaml:2]
- [x] [Review][Patch] [HIGH] Make the Action pin-update procedure require an explicit capability-registry classification backed by manifest, endpoint, Action tests, and smoke-consumer evidence; following the current documented steps leaves the new pin unclassified and breaks scaffold generation. [docs/github-action.md:104]
- [x] [Review][Patch] [HIGH] Do not unconditionally recommend a `github-action` advisory override for enforcement-capable onboarding when an existing repository shares the same project/integration identity; require a separate project/integration identity or a completed scope review before attachment. [integrations/github/init_service.py:398]
- [x] [Review][Patch] [HIGH] Keep policy blocks advisory during enforcement-capable onboarding without turning authentication failures, timeouts, malformed decisions, or Action crashes into a green job; add explicit terminal handling and structurally test the rendered Action step and failure paths. [integrations/github/init_service.py:313]
- [x] [Review][Patch] [HIGH] Require consumers to validate allowed values and cross-field invariants for `policy-status`, `configured-mode`, and `effective-status` as well as `should-block`; a present but contradictory output set must fail operationally. [docs/github-action.md:145]
- [x] [Review][Patch] [MEDIUM] Correct the mode-table promise that every valid non-blocking decision exits `0`; the policy result may be non-blocking while later output publication, comment update, or another operational step still fails. [docs/enforcement-guardrails.md:26]
- [x] [Review][Patch] [MEDIUM] Define an observable operational-error classification distinct from `hard-block` even when both produce a nonzero Action exit or GitHub App `failure`, including the check summary/log evidence operators use to distinguish them. [docs/enforcement-guardrails.md:24]
- [x] [Review][Patch] [HIGH] Expand the complete approval identity with repository/environment, integration and project scope, and protected-target identity so approval for one delivery scope cannot authorize another scope sharing the same report fields. [docs/enforcement-guardrails.md:504]
- [x] [Review][Patch] [MEDIUM] Define versioned deterministic serialization and digest algorithms for manifest and settings snapshots, not only material context, so approval identities are reproducible across consumers. [docs/enforcement-guardrails.md:504]
- [x] [Review][Patch] [MEDIUM] Retain the complete decision identity plus manifest and material-context snapshots with audit evidence; retaining only a report ID and decision response becomes non-reconstructible when report storage expires first. [docs/enforcement-guardrails.md:322]
- [x] [Review][Patch] [MEDIUM] Parse indented GFM table rows and rows without an optional leading pipe as part of the contiguous effective-status table; currently a rendered contradictory row can terminate parsing and evade regression coverage. [tests/test_docs/test_enforcement_guardrails.py:975]
- [x] [Review][Patch] [MEDIUM] Either parse reference-style/local Markdown links or explicitly reject unsupported rendered link forms; validating only destinations recognized by the custom inline-link parser can silently omit newly broken outbound links. [tests/test_docs/test_enforcement_guardrails.py:759]
- [x] [Review][Patch] [MEDIUM] Reject an unclosed YAML workflow fence before parsing its body; an unterminated documentation block currently satisfies the structured workflow contract. [tests/test_docs/test_enforcement_guardrails.py:905]
- [x] [Review][Patch] [LOW] Normalize inline Markdown and entities when computing GitHub heading slugs so fragment checks neither include link destinations nor accept invalid anchors. [tests/test_docs/test_enforcement_guardrails.py:799]
- [x] [Review][Patch] [HIGH] Validate the complete policy-output tuple on both successful and failed enforcement-capable Action runs, and require an explicit validated-policy-block discriminator before converting failure to an advisory warning; `should-block=true` alone can currently hide malformed decisions or post-decision operational failures. [integrations/github/init_service.py:315]
- [x] [Review][Patch] [HIGH] Enforce reviewed immutable 40-character SHA pins at scaffold runtime for both DeployWhisper Analyze Action and checkout; registry membership or self-referential test expectations must not admit a moving or malformed executable reference. [integrations/github/init_service.py:50]
- [x] [Review][Patch] [MEDIUM] Reconcile the README's unconditional `0` exit promise for valid non-blocking decisions with the guardrail guide's explicit output-publication, comment-delivery, authentication, timeout, and other post-decision failure paths. [README.md:770]
- [x] [Review][Patch] [HIGH] Include the immutable DeployWhisper application/server revision, dependency identity, endpoint-contract version, and validated decision digest in freshness and complete approval identity so a server rollout cannot reuse stale human approval. [docs/enforcement-guardrails.md:184]
- [x] [Review][Patch] [HIGH] Make rollout readiness require the external approval record or independently enforced approval check that can invalidate non-commit identity changes; native repository/environment approval alone cannot bind report, manifest, settings, or context digests. [docs/enforcement-guardrails.md:600]
- [x] [Review][Patch] [MEDIUM] Require manifest completeness to compare a canonical unique normalized path-to-content-hash mapping with the trusted changed-file set, including explicit case and alias handling; counter equality alone permits duplicate or omitted paths. [docs/enforcement-guardrails.md:209]
- [x] [Review][Patch] [MEDIUM] Add the below-`high` blocking prohibition to the normative rollout checklist so recording thresholds and generic Evidence Law expectations cannot authorize unsupported lower-threshold enforcement. [docs/enforcement-guardrails.md:584]
...
- 2026-09-17 review rerun RED: strengthened contracts reproduced the remaining capability-classification, structured-workflow, break-glass, approval-freshness, audit-integrity, lifecycle, Markdown-parser, and rollout-readiness gaps (`12 failed, 23 passed, 120 subtests passed`).
- 2026-09-17 review rerun focused GREEN: `./.venv/bin/python -m pytest tests/test_docs/test_enforcement_guardrails.py tests/test_services/test_github_init_service.py -q --tb=short` - `26 passed, 129 subtests passed`.
- 2026-09-17 expanded documentation, metadata, and scaffold suite: `./.venv/bin/python -m pytest tests/test_docs tests/test_infra/test_ai_safety_documentation.py tests/test_infra/test_requirements_traceability_matrix.py tests/test_services/test_github_init_service.py -q --tb=short` - `63 passed, 322 subtests passed`.
- 2026-09-17 required smoke: `./.venv/bin/python -m unittest discover -q` passed with exit code 0.
- 2026-09-17 full local CI: `bash scripts/ci-local.sh` passed Ruff, dependency integrity, Bandit with zero high-severity findings, compileall, skill and prompt-injection gates, and every backend/docs test directory; the final services directory reported `931 tests` passing.
- 2026-09-17 quality gates: `./.venv/bin/ruff check .`, repo-wide `./.venv/bin/ruff format --check .`, and `git diff --check` passed; Ruff reported all 273 files formatted.
- 2026-09-18 review rerun RED: rendered-Markdown, capability-branch, inherited-mode, timeout/cancellation, approval-identity, malformed-output, and exception-overlap regressions reproduced all ten findings (`26 failed, 22 passed, 119 subtests passed`).
- 2026-09-18 review rerun focused GREEN: `./.venv/bin/python -m pytest tests/test_docs/test_enforcement_guardrails.py tests/test_services/test_github_init_service.py -q --tb=short` - `30 passed, 137 subtests passed`.
- 2026-09-18 expanded documentation, metadata, and scaffold suite: `./.venv/bin/python -m pytest tests/test_docs tests/test_infra/test_ai_safety_documentation.py tests/test_infra/test_requirements_traceability_matrix.py tests/test_services/test_github_init_service.py -q --tb=short` - `67 passed, 330 subtests passed`.
- 2026-09-18 required smoke: `./.venv/bin/python -m unittest discover -q` - `433 tests` passed, `1 skipped`.
- 2026-09-18 full local CI: `bash scripts/ci-local.sh` passed Ruff, dependency integrity, Bandit with zero high-severity findings, compileall, skill and prompt-injection gates, and every backend/docs test directory; the final services directory reported `933 tests` passing.
- 2026-09-18 quality gates: `./.venv/bin/ruff check .`, repo-wide `./.venv/bin/ruff format --check .`, and `git diff --check` passed; Ruff reported all 273 files formatted.
- 2026-09-18 UI validation not applicable: no React route, component, rendered surface, interaction, keyboard behavior, or accessibility semantics changed.
- 2026-09-21 review rerun RED: new capability-value, enforcement-onboarding, rollout-readiness, identity/digest, retention, Markdown-rendering, structural-duplicate, and all-local-link regressions reproduced the ten findings (`14 failed, 26 passed, 141 subtests passed`).
- 2026-09-21 review rerun focused GREEN: `./.venv/bin/python -m pytest tests/test_services/test_github_init_service.py tests/test_docs/test_enforcement_guardrails.py -q --tb=short` - `32 passed, 149 subtests passed`.
- 2026-09-21 expanded documentation, metadata, and scaffold suite: `./.venv/bin/python -m pytest tests/test_docs tests/test_infra/test_ai_safety_documentation.py tests/test_infra/test_requirements_traceability_matrix.py tests/test_services/test_github_init_service.py -q --tb=short` - `69 passed, 342 subtests passed`.
- 2026-09-21 required smoke: `./.venv/bin/python -m unittest discover -q` passed with exit code 0.
- 2026-09-21 full local CI: `bash scripts/ci-local.sh` passed Ruff, dependency integrity, Bandit with zero high-severity findings, compileall, skill and prompt-injection gates, and every backend/docs test directory; the final services directory reported `934 tests` passing.
- 2026-09-21 quality gates: `./.venv/bin/ruff check .`, repo-wide `./.venv/bin/ruff format --check .`, and `git diff --check` passed; Ruff reported all 273 files formatted.
- 2026-09-21 UI validation not applicable: no React route, component, rendered surface, interaction, keyboard behavior, or accessibility semantics changed.
- 2026-09-21 post-fix review RED: sprint-metadata, workflow-structure, scope-onboarding, output-invariant, operational-classification, approval/audit-identity, GFM table/link/fence, and heading-slug contracts reproduced all fourteen findings (`20 failed, 37 passed, 222 subtests passed`).
- 2026-09-21 post-fix review focused GREEN: `./.venv/bin/python -m pytest tests/test_services/test_github_init_service.py tests/test_docs/test_enforcement_guardrails.py tests/test_infra/test_ai_safety_documentation.py -q --tb=short` - `43 passed, 236 subtests passed`.
- 2026-09-21 post-fix expanded documentation, metadata, and scaffold suite: `./.venv/bin/python -m pytest tests/test_docs tests/test_infra/test_ai_safety_documentation.py tests/test_infra/test_requirements_traceability_matrix.py tests/test_services/test_github_init_service.py -q --tb=short` - `70 passed, 357 subtests passed`.
- 2026-09-21 post-fix required smoke: `./.venv/bin/python -m unittest discover -q` - `435 tests` passed, `1 skipped`.
- 2026-09-21 post-fix full local CI: `bash scripts/ci-local.sh` passed Ruff, dependency integrity, Bandit with zero high-severity findings, compileall, skill and prompt-injection gates, and every backend/docs test directory; the final services directory reported `934 tests` passing.
- 2026-09-21 post-fix UI validation not applicable: no React route, component, rendered surface, interaction, keyboard behavior, or accessibility semantics changed.
- 2026-09-25 review rerun RED: complete-output/discriminator, immutable-pin, README-exit, approval-identity, manifest/idempotency, threshold/decision-metric, and unsupported-Markdown contracts reproduced all ten actionable findings (`13 failed, 30 passed, 29 subtests passed`).
- 2026-09-25 review rerun focused GREEN: `./.venv/bin/python -m pytest tests/test_services/test_github_init_service.py tests/test_docs/test_enforcement_guardrails.py tests/test_docs/test_github_action_integration_contract.py -q --tb=short` - `44 passed, 215 subtests passed`.
- 2026-09-25 expanded documentation, metadata, and scaffold suite: `./.venv/bin/python -m pytest tests/test_docs tests/test_infra/test_ai_safety_documentation.py tests/test_infra/test_requirements_traceability_matrix.py tests/test_services/test_github_init_service.py -q --tb=short` - `73 passed, 378 subtests passed`.
...
- Resolved all 14 findings from the 2026-09-17 rerun: Action pins now require explicit capability classification; workflow examples are parsed structurally; break-glass, publication failure, audit bytes, and human approval have fail-safe lifecycle controls; blocking-readiness exceptions cannot masquerade as satisfied prerequisites; checkout refs are peeled; Epic 11 status is validated bidirectionally from parsed YAML; and Markdown helpers reject fence, anchor, section, and duplicate-table escapes.
- Resolved all ten findings from the 2026-09-18 rerun: capability selection now fails closed independently of pin edits; generated README and PR guidance cover advisory and enforcement-capable revisions; inherited settings, cancellation, unknown submission outcomes, approval identity, malformed Action outputs, and overlapping exceptions have explicit fail-safe controls; and documentation contracts now validate rendered Markdown and bounded table structure.
...
- 2026-09-17: Addressed all 14 findings from the latest review rerun, completed focused and repository-wide validation, and retained Story 11.4 and Epic 11 as done.
- 2026-09-18: Addressed all ten findings from the latest review rerun, completed focused and repository-wide validation, and retained Story 11.4 and Epic 11 as done.
