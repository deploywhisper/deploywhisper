...
def extract_init_script_params(script_path: Path) -> dict:
    ...
    if tmpl_match:
        params["template_files"] = re.findall(
            r'["\']([^"\']+)["\']', tmpl_match.group(1)
        )

    # SKILL_ONLY_FILES
    only_match = re.search(r"SKILL_ONLY_FILES\s*=\s*\{(.*?)\}", content, re.DOTALL)
    if only_match:
        params["skill_only_files"] = re.findall(
            r'["\']([^"\']+)["\']', only_match.group(1)
        )
...
def main():
    ...
    for tmpl in STANDARD_TEMPLATES:
        ...
        template_inventory[tmpl] = {
            ...
            "content_lines": count_content_lines(tmpl_path)
            if tmpl_path.exists()
            else 0,
        }
    ...
    fb_sections = (
        extract_h2_h3_sections(first_breath_path) if first_breath_path.exists() else []
    )
    ...
    is_configuration = (
        check_section_present(fb_sections, "Discovery") and not is_calibration
    )
    fb_style = (
        "calibration"
        if is_calibration
        else ("configuration" if is_configuration else "unknown")
    )

    expected_sections = (
        FIRST_BREATH_CALIBRATION_SECTIONS
        if is_calibration
        else FIRST_BREATH_CONFIG_SECTIONS
    )
    fb_check = {}
    ...
    skill_name_match = (
        init_params.get("skill_name") == skill_path.name
        if init_params["exists"]
        else None
    )
