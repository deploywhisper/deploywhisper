...
- Phase 7 UI migration cutover: React now serves as the root web UI at `/`, legacy `/app/...` links redirect to root routes, NiceGUI code/tests/dependencies were removed, and the runtime image no longer copies the `ui/` package. Final local image size moved from 310 MB to 210 MB.
